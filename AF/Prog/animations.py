...
def create_rocket_trail(position, shift, update=False):
...
def create_smoke(position, shift, update=False):
    ...
    if update:
        ...
        # количество создаваемых частиц
        particle_count = random.randint(-1, 1)
        
        # возможные скорости
        numbers = range(-100, -20)
        if particle_count > 0:
            for _ in range(particle_count):
                Particle(position, random.choice(numbers)/100, -2, smoke_cloud, smoke_sprites, random.randint(250, 400)/100)
