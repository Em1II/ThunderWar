...
from Prog import rot, vis, techsounds, animations
import screeninfo


class PLANE:
    def __init__(self, screen):
        self.plane = pygame.image.load('img/img;tech/plane1.png').convert_alpha()
        self.plane_loc= Vector2(100, 100)
        ...
        self.HP = 1000
        self.sc = screen
        self.hitbox = {'top_right': Vector2(self.plane_loc.x + 10, self.plane_loc.y + 5),
                       'down_left':Vector2(self.plane_loc.x - 10, self.plane_loc.y - 5)}
        self.speed = 0
        self.angel = 0
    def hurt(self, hurt):
        self.HP -= hurt
    def update_hitbox(self):
        self.hitbox = {'top_right': Vector2(self.plane_loc.x + 0, self.plane_loc.y + 25),
                       'down_left': Vector2(self.plane_loc.x - 40, self.plane_loc.y - 25)}
    def plane_draw(self):
        self.loc_update()
        if self.vs.get_game_cond() != 'F':
            rect = pygame.Rect((self.plane_loc.x, self.plane_loc.y, 30, 30))
            self.sc.blit(rot.rott(self.angel, self.plane), rect)
    #plane dir
    def get_horllvert_speed(self, speed, angel):
        angel = (angel + 360) % 360
    ...
    def plane_dest(self, x, y, ground=False):
        techsounds.play_exlosion()
        animations.explosion_of_aircraft(self.sc, x, y, ground)
        self.vs.game_over()
    def fail_check(self):
        if self.HP <= 0:
            self.plane_dest(self.plane_loc.x, self.plane_loc.y)
            self.speed = 0
    def loc_update(self):
        self.fail_check()
        k = self.get_horllvert_speed(self.speed, self.angel)
        self.plane_loc.x += k[0]
        self.plane_loc.y += k[1]
        m = screeninfo.get_monitors()[0]
        if m.height < self.plane_loc.y + 130:
            x = self.plane_loc.x - 80
            y = self.plane_loc.y - 60
            self.plane_dest(x, y, ground = True)
        elif m.width < self.plane_loc.x:
            self.plane_loc.x = -1
            self.angel = 0
        elif self.plane_loc.x < 0:
            self.plane_loc.x = m.width
            self.angel = 180
        self.update_hitbox()
    #plane move
